...
public class BulletController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isEnemyBullet)
        {
            ...
            transform.position = Vector2.MoveTowards(transform.position, playerPos - curPos, 5f * Time.deltaTime);
            if (curPos == lastPos)
            {
                Destroy(gameObject);
            }
    ...
    {
        if (other.tag == "EnemyHitbox" && !isEnemyBullet)
        {
        ...
        } else if (other.tag != "Enemy" && !isEnemyBullet && other.tag != "Water")
        {
            Destroy(this.gameObject);
        } 
